...
import "./interfaces/IVaultAuthorizer.sol";
 ...
 */
abstract contract RequiemVault is FlashLoans, Swaps {

    address private vaultAuthorizer;

    constructor(
        address _vaultAuthorizer,
        IWETH weth,
        uint256 pauseWindowDuration,
        uint256 bufferPeriodDuration
    ) AssetHelpers(weth) TemporarilyPausableVault(pauseWindowDuration, bufferPeriodDuration) TemporarilyPausableFlashLoan(pauseWindowDuration, bufferPeriodDuration) {
        // solhint-disable-previous-line no-empty-blocks
        vaultAuthorizer = _vaultAuthorizer;
    }

    // modifier authenticateFor(address user) override {
    //     IVaultAuthorizer(vaultAuthorizer)._authenticateFor(user);
    //     _;
    // }

    function setAuthorizer(IAuthorizer newAuthorizer) external override nonReentrant authenticate {
        IVaultAuthorizer(vaultAuthorizer).setAuthorizer(newAuthorizer);
    }

    function hasApprovedRelayer(address user, address relayer) external view override returns (bool) {
        return IVaultAuthorizer(vaultAuthorizer).hasApprovedRelayer(user, relayer);
    }

    function getAuthorizer() external view override returns (IAuthorizer) {
        return IVaultAuthorizer(vaultAuthorizer).getAuthorizer();
    }
    ...
    function setRelayerApproval(
        ...
        IVaultAuthorizer(vaultAuthorizer).setRelayerApproval(sender, relayer, approved);
    }
